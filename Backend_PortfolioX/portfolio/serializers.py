...
# ============================================================================
# SERIALIZERS DE BASE - CRUD COMPLET
# ============================================================================
...
# ============================================================================
# SERIALIZERS SPÉCIAUX POUR LES ACTIONS PORTFOLIO
# ============================================================================
...
class PortfolioPublierSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not portfolio.titre:
            raise serializers.ValidationError({'titre': 'Le portfolio doit avoir un titre pour être publié.'})
        if not portfolio.template:
            raise serializers.ValidationError({'template': 'Un template doit être sélectionné pour publier le portfolio.'})
        if portfolio.competences.count() == 0:
            raise serializers.ValidationError({'competences': 'Le portfolio doit contenir au moins une compétence.'})
        return data
...
class PortfolioAjouterProjetSerializer(serializers.ModelSerializer):
    ...
    projet_existant_id = serializers.PrimaryKeyRelatedField(
        queryset=Projet.objects.all(), write_only=True, required=False, source='projets'
    )
    ...
    def validate(self, data):
        if not data.get('nouveau_projet') and not data.get('projets'):
            raise serializers.ValidationError("Vous devez fournir soit un nouveau projet, soit un projet existant.")
        return data
...
class PortfolioAjouterCompetenceSerializer(serializers.ModelSerializer):
    ...
    competence_existante_id = serializers.PrimaryKeyRelatedField(
        queryset=Competence.objects.all(), write_only=True, required=False, source='competences'
    )
    ...
    def validate(self, data):
        if not data.get('nouvelle_competence') and not data.get('competences'):
            raise serializers.ValidationError("Vous devez fournir soit une nouvelle compétence, soit une compétence existante.")
        return data
...
class PortfolioAjouterContactSerializer(serializers.ModelSerializer):
    ...
    contact_existant_id = serializers.PrimaryKeyRelatedField(
        queryset=Contact.objects.all(), write_only=True, required=False, source='contacts'
    )
    ...
    def validate(self, data):
        if not data.get('nouveau_contact') and not data.get('contacts'):
            raise serializers.ValidationError("Vous devez fournir soit un nouveau contact, soit un contact existant.")
        return data
...
# ============================================================================
# SERIALIZERS PRINCIPAUX PORTFOLIO - CRUD COMPLET
# ============================================================================
...
class PortfolioListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Portfolio
        fields = [
            'id_portfolio', 'titre', 'utilisateur_nom', 'template_nom', 'statut',
            'date_creation', 'nombre_competences', 'nombre_projets', 'nombre_contacts'
        ]
        read_only_fields = ['date_creation']
    
    def get_nombre_competences(self, obj): return obj.competences.count()
    def get_nombre_projets(self, obj): return obj.projets.count()
    def get_nombre_contacts(self, obj): return obj.contacts.count()
...
class PortfolioDetailSerializer(serializers.ModelSerializer):
    ...
    competences_ids = serializers.PrimaryKeyRelatedField(
        queryset=Competence.objects.all(), many=True, write_only=True, source='competences', required=False
    )
    contacts_ids = serializers.PrimaryKeyRelatedField(
        queryset=Contact.objects.all(), many=True, write_only=True, source='contacts', required=False
    )
    projets_ids = serializers.PrimaryKeyRelatedField(
        queryset=Projet.objects.all(), many=True, write_only=True, source='projets', required=False
    )
    
    class Meta:
        model = Portfolio
        fields = [
            'id_portfolio', 'utilisateur', 'template', 'titre', 'date_creation', 
            'description', 'statut', 'competences', 'contacts', 'projets',
            'competences_ids', 'contacts_ids', 'projets_ids'
        ]
...
class PortfolioCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Portfolio
        fields = ['utilisateur', 'template', 'titre', 'description', 'statut', 'competences', 'contacts', 'projets']
...
class PortfolioPublicSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Portfolio
        fields = [
            'id_portfolio', 'titre', 'description', 'utilisateur_nom', 'template_nom',
            'competences', 'contacts', 'projets', 'date_creation'
        ]
        read_only_fields = fields
    
    def get_utilisateur_nom(self, obj): return f"{obj.utilisateur.prenom} {obj.utilisateur.nom}"

# ============================================================================
# SERIALIZERS POUR OPÉRATIONS AVANCÉES
# ============================================================================

class PortfolioUpdateSerializer(serializers.ModelSerializer):
    """Serializer pour la mise à jour complète d'un portfolio"""
    class Meta:
        model = Portfolio
        fields = ['titre', 'description', 'statut', 'template']
    
    def validate(self, data):
        if data.get('statut') == 'publie':
            portfolio = self.instance
            if not portfolio.titre:
                raise serializers.ValidationError({'titre': 'Un titre est requis pour publier le portfolio.'})
            if not portfolio.template:
                raise serializers.ValidationError({'template': 'Un template est requis pour publier le portfolio.'})
        return data

class PortfolioPartialUpdateSerializer(serializers.ModelSerializer):
    """Serializer pour la mise à jour partielle"""
    class Meta:
        model = Portfolio
...
# ============================================================================
# SERIALIZERS POUR GESTION DES RELATIONS
# ============================================================================

class PortfolioManageCompetencesSerializer(serializers.ModelSerializer):
    """Serializer pour gérer les compétences d'un portfolio"""
    competences = serializers.PrimaryKeyRelatedField(queryset=Competence.objects.all(), many=True)
    
    class Meta:
        model = Portfolio
        fields = ['competences']

class PortfolioManageProjetsSerializer(serializers.ModelSerializer):
    """Serializer pour gérer les projets d'un portfolio"""
    projets = serializers.PrimaryKeyRelatedField(queryset=Projet.objects.all(), many=True)
    
    class Meta:
        model = Portfolio
        fields = ['projets']

class PortfolioManageContactsSerializer(serializers.ModelSerializer):
    """Serializer pour gérer les contacts d'un portfolio"""
